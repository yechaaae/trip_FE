  ...
  <div class="modal-overlay" v-if="isVisible" @click.self="closeModal">
    <div class="modal-content">
      ...
      <ul class="result-list">
        <li v-if="results.length === 0 && keyword.length > 0" class="no-result">
           검색 결과가 없습니다.
        </li>
...
.search-box button { padding: 8px 12px; background: #eee; border: 1px solid #ccc; border-radius: 4px; cursor: pointer; }

.result-list { 
  list-style: none; padding: 0; margin: 0; 
  overflow-y: auto; flex: 1; border-top: 1px solid #eee;
}
.result-item {
...
.addr { font-size: 12px; color: #888; margin-top: 2px; }
.no-result { padding: 20px; text-align: center; color: #999; }

.close-btn { margin-top: 10px; padding: 10px; background: #333; color: white; border: none; border-radius: 4px; cursor: pointer; }
</style>